...
from src.config import SLACK_WEBHOOK_URL, SLACK_CHANNEL, SLACK_ENABLED, DRY_RUN, AUTOMATION_ID
...
class SlackNotifier:
    ...
    def _format_message(self, message: str, level: str, details: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def send_info(self, message: str, details: Optional[Dict[str, Any]] = None):
        ...
        if self.enabled or DRY_RUN:  # Always log in DRY_RUN mode
            ...
            logger.info(f"Slack notification: {message}", extra=details or {})
    ...
    def send_warning(self, message: str, details: Optional[Dict[str, Any]] = None):
        ...
        if self.enabled or DRY_RUN:
            ...
            logger.warning(f"Slack notification: {message}", extra=details or {})
            
            return self._send_to_slack(payload)
    
    def send_error(self, message: str, details: Optional[Dict[str, Any]] = None, request_id: Optional[str] = None):
        ...
        if self.enabled or DRY_RUN:
            ...
            logger.error(f"Slack notification: {message}", extra=error_details)
    ...
    def send_refund_summary(self, successful_refunds: int, failed_refunds: int, total_amount: float, currency: str):
        ...
        details = {
            ...
            "Total Refunded": f"{total_amount} {currency}" if successful_refunds > 0 else "0",
        ...
        if self.enabled or DRY_RUN:
            ...
            logger.info(f"Slack notification (SUMMARY): {message}", extra=details)
