...
class TestIdempotency:
    ...
    def test_idempotency_is_duplicate_operation(self, sample_order):
    ...
    def test_save_cache(self, mock_file):
        ...
        idempotency_manager._cache = test_cache
        
        idempotency_manager._save_cache()
    ...
    # @patch("src.utils.idempotency.idempotency_manager._cleanup_expired_entries")
    # @patch('builtins.open', new_callable=mock_open, read_data='{"test_key": {"timestamp": "2025-01-01T00:00:00", "operation": "refund"}}')
    # def test_load_cache_success(self, mock_file, mock_cleanup_expired_entries):

    #     # Create a fresh manager and mock the config to avoid initialization calls
    ...
    #     # Verify file was opened correctly - look for the read call specifically
    #     assert any(call[0][1] == "r" for call in mock_file.call_args_list)
    ...
    #     mock_cleanup_expired_entries.assert_called_once()
