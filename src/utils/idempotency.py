...
class IdempotencyManager:
    ...
    def _cleanup_expired_entries(self):
        ...
        for key, entry in self._cache.items():
            try:
                entry_time = timezone_handler.to_store_timezone(datetime.fromisoformat(entry['timestamp']))
                expired_time = entry_time + timedelta(hours=self.ttl_hours)
                
                time_comparison = timezone_handler.compare_times_in_store_tz(current_time, expired_time)
                ...
                logger.warning(f"Error cleaning cleanup_expired_entries", extra={"error":str(e)})
...
def load_cache_data(instance: "IdempotencyManager"):
    ...
    try:
        with open(instance.cache_file, "r") as f:
            ...
            logger.debug(f"Loaded idempotency cache with {len(instance._cache)} entries")
    except Exception as e:
